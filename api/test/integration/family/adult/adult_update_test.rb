...
class Family::AdultUpdateTest < ActionDispatch::IntegrationTest
  test 'adult update success' do
    ...
    name = Faker::Name.name
    photo_url = Faker::Internet.url
    put "/api/v1/family/adult/#{id}", token: token, name: name, photo_url: photo_url
  ...
  test 'adult update only one param success' do
    ...
    # action 1
    name = Faker::Name.name
    put "/api/v1/family/adult/#{id}", token: token, name: name
    ...
    # action 2
    photo_url = Faker::Internet.url
    put "/api/v1/family/adult/#{id}", token: token, photo_url: photo_url
